...
def metrics(model, x, indices, deltas, epsilon, k = None):
...
def eval_epsilon(model, x, indices, epsilon):
    
    input_dim = x.shape[1]
    num_clusters = len(indices)

    a, b = metrics(model, x, indices, np.zeros((num_clusters - 1, input_dim)), epsilon)
